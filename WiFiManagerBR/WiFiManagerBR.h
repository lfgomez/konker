/**************************************************************
 * WiFiManagerBR é um Fork da biblioteca WiFiManager (https://github.com/tzapu/WiFiManager)
 * que se utiliza da Plataforma ESP8266/Arduino encontrada em (https://github.com/esp8266/Arduino)
 * Nesse Fork, a biblioteca foi modificada para saídas em Português (em HTML, sendo que o debug serial se mantem em inglês).
 * Algumas modificações estéticas e funcionais também foram feitas nessa versão, porém mantendo total compatibilidade com a bilbioteca original
 * Licenciado sob Licença MIT (Licensed under MIT license)
 * Responsável: Luis Fernando Gomez Gonzalez (luis.gonzalez@inmetrics.com.br)
 **************************************************************/


#ifndef WiFiManagerBR_h
#define WiFiManagerBR_h
...
const char HTTP_PORTAL_OPTIONS[] PROGMEM = "<form action=\"/wifi\" method=\"get\"><button>Configurar WiFi</button></form><br/><form action=\"/0wifi\" method=\"get\"><button>Configurar WiFi (Sem Busca)</button></form>";
...
const char HTTP_FORM[] PROGMEM = "<form method='get' action='wifisave'><input id='s' name='s' length=32 placeholder='SSID'><br/><input id='p' name='p' length=64 placeholder='senha'><br/><br/><button type='submit'>Salvar</button></form>";
