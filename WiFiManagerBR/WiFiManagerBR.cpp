...
void WiFiManager::handleRoot() {
  ...
  head.replace("{v}", "P&aacutegina de Configura&ccedil&atildeo");
  ...
  server->sendContent(F("<h3>Configura&ccedil&atildeo de Rede</h3>"));
  ...
  if (scan) {
    ...
    if (n == 0) {
      ...
      server->sendContent("Nenhuma rede encontrada. Por favor atualize a p&aacutegina para repetir a busca.");
       ...
       server->sendContent(F("no campo [senha].</h3><br><br>"));
      for (int i = 0; i < n; ++i)
      {
...
void WiFiManager::handleWifiSave() {
  ...
  String head = HTTP_HEAD;
  head.replace("{v}", "Rede Wifi Salva");
  server->sendContent(head);
  server->sendContent_P(HTTP_SCRIPT);
  server->sendContent_P(HTTP_STYLE);
  server->sendContent_P(HTTP_HEAD_END);
